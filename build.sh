...
AK3_BRANCH="$DEVICE-aospksu"
AK3_DIR="$(pwd)/android/AnyKernel3"

ZIPNAME="aospKSU-$(date '+%Y%m%d').zip"
TC_DIR="$(pwd)/tc/clang"
DEFCONFIG="vendor/kona-perf_defconfig vendor/samsung/kona-sec-common.config vendor/samsung/$DEVICE.config vendor/ksu.config"
...
if ! [ -d "$TC_DIR" ]; then
    ...
    mkdir -p "$TC_DIR"
    if ! curl -L https://www.kernel.org/pub/tools/llvm/files/llvm-22.1.5-x86_64.tar.gz \
